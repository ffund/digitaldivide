...
for trial in $(seq 0 10); do
  ...
  iperf3 -c server -i 5 -t 60 -P 3 -w 50k --format k        > "ulrate-$trial.txt"
  iperf3 -c server -i 5 -t 60 -P 3 -w 50k -R --format k     > "dlrate-$trial.txt"

  # Test UL/DL jitter and packet loss
  # Server must be running 'iperf -s -u'
  echo '[' `date` ']' " Jitter/loss test for trial $trial"
  iperf -c server -u -b 100k -t 60 -l 180 -r  > "udp-$trial.txt"
done
